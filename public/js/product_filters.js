var filterStartPrice;
var filterEndPrice;
var filterColor;
var filterCountry;
var filterAppellation;
var filterProducer;
var filterEnvironment;
var filterFormat;
var filterVarietals;
var filter_Type_Category;
var filter_Rice_Type;
var filter_Rice_Polishing_Rate;
...
function rangeFilter(From, To) {
    filterStartPrice = From;
    filterEndPrice = To;
    $('.products_view').each(function (i, obj) {
        var item_price_regular = $(this).attr('item_price_regular');
        if (From <= item_price_regular || item_price_regular >= To) {
            $(this).show();
        } else {
            $(this).hide();
        }
...
function CountryPicker(Country, id) {

    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Country_Filters').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');
        filterCountry = Country;
        ajaxCallFilter();
    } else {
        filterCountry = nullVal;
        $('#' + id).removeClass('filterTextBold');
        ajaxCallFilter();
    }
}

function Type_Category(Type_Category, id) {
    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Type_Category').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');
        filter_Type_Category = Type_Category;
        ajaxCallFilter();
    } else {
        filter_Type_Category = nullVal;
        $('#' + id).removeClass('filterTextBold');
        ajaxCallFilter();
    }
}

function Rice_TypePicker(Rice_Type, id) {
    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Rice_Type').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');
        filter_Rice_Type = Rice_Type;
        ajaxCallFilter();
    } else {
        filter_Rice_Type = nullVal;
        $('#' + id).removeClass('filterTextBold');
        ajaxCallFilter();
    }
}

function Rice_Polishing_RatePicker(Rice_Polishing_Rate, id) {
    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Rice_Type').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');
        filter_Rice_Polishing_Rate = Rice_Polishing_Rate;
        ajaxCallFilter();
    } else {
        filter_Rice_Polishing_Rate = nullVal;
        $('#' + id).removeClass('filterTextBold');
        ajaxCallFilter();
    }
}

function ColorPicker(color, id) {

    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Colour_Filters').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');
        filterColor = color;
        ajaxCallFilter();
    } else {
...
function AppellationPicker(Appellation, id) {

    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Appellation_Filters').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');
        filterAppellation = Appellation;
        ajaxCallFilter();
    } else {
        filterAppellation = nullVal;
        $('#' + id).removeClass('filterTextBold');
        ajaxCallFilter();
    }
}
...
function ProducerPicker(Producer, id) {

    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Producer_Filters').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');
        filterProducer = Producer;
        ajaxCallFilter();
    } else {
        filterProducer = nullVal;
        $('#' + id).removeClass('filterTextBold');
        ajaxCallFilter();
    }
}
...
function VarietalsPicker(Varietals, id) {

    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Varietals_Filters').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');

        filterVarietals = Varietals;
        ajaxCallFilter();
    } else {
        filterVarietals = nullVal;
        $('#' + id).removeClass('filterTextBold');
        ajaxCallFilter();
    }
}
...
function FormatPicker(Format, id) {

    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Format_Filters').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');
        filterFormat = Format;
        ajaxCallFilter();
    } else {
        filterFormat = nullVal;
        $('#' + id).removeClass('filterTextBold');
        ajaxCallFilter();
    }
}
...
function EnvironmentPicker(Environment, id) {

    if (!$('#' + id).hasClass('filterTextBold')) {
        $('.Environment_Filters').removeClass('filterTextBold');
        $('#' + id).addClass('filterTextBold');
        filterEnvironment = Environment;
        ajaxCallFilter();
    } else {
        filterEnvironment = nullVal;
        $('#' + id).removeClass('filterTextBold');
        ajaxCallFilter();
        console.log(nullVal);
    }
...
function ajaxCallFilter() {
    ...
    $.ajax({
        ...
        data: {
            ...
            filterAppellation: filterAppellation,
            filterProducer: filterProducer,
            filterEnvironment: filterEnvironment,
            filterFormat: filterFormat,
            filterVarietals: filterVarietals,
            filter_Type_Category: filter_Type_Category,
            filter_Rice_Type: filter_Rice_Type,
            filter_Rice_Polishing_Rate: filter_Rice_Polishing_Rate,
            category: category,
        },
        error: function () {
        ...
        success: function (data) {
           ...
           var html = '';
            data.forEach(i => {
                html +='<div class="products_view" product_color="'+i.item_color+'" item_price_regular="'+i.g_regular_price+'">';
                html +='<a class="product-cat-pg-link" href="/shop/product/'+i.id+'/'+i.name+'">';
                html +='<div class="single-item">';
                html +='<div class="col-md-6"';
                  html +='style="text-align: center;font-size: 14px;padding-top: 14px;padding-bottom: 12px; max-width: initial;">';
                    if(i.product_thumbnail_image != null){
                 html +='<img src="/storage/'+i.product_thumbnail_image+'"';
                  html +='class="img-grid-view" />';
            }else{
                            html +='<img src="\static\Image-2-1.jpg" class="img-grid-view" />';
                        }    

                            html += '<h1>'+i.name+'</h1>';
                            html += '<p class="portfolio-product-price">';
                            html += '$'+i.g_regular_price+'</p>';
                                html +='</div>';
                                html +='</div>';
                                html +='</a>';
                                html +='</div>';
            ...
            if(filterStartPrice>0 || filterEndPrice>0){
                rangeFilter(filterStartPrice, filterEndPrice);
            }
